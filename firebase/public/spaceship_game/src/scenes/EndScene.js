...
class EndScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.cache.game.db.collection("tasks").doc('virus_study_spaceship_gameT9').collection('subjects').doc('completed_subs').update({
            completed_subs: firebase.firestore.FieldValue.arrayUnion(this.cache.game.subjectID),
            time_taken: firebase.firestore.FieldValue.arrayUnion(time_taken),
        ...
        docRef.update({
            trial_data: firebase.firestore.FieldValue.arrayUnion(this.cache.game.data)
        }).then(
    ...
    createPreamble(preamble) {
        ...
        var p = document.createElement("p");
        p.setAttribute("class", "instructionCheckText");
    ...
    getValues() {
        ...
        if (incomplete.length > 0) {
            ...
            docRef.update({
                ...
                if (condition == 'SPACESHIPPILOT') {
                    window.location.href = 'https://app.prolific.co/submissions/complete?cc=3056D9CC';
                }
